    ...
    public static partial class CfgHelper
    {
        ...
        public sealed class Table<TItem> : ITable<TItem> where TItem : CfgTableItemBase
        {
            ...
            public struct ConstructData
            {
                public string? Id;
                public TItem[]? Items;
                public IReadOnlyDictionary<Id, TItem>? Dict;
            }
            ...
            public Table(TableGroup<TItem> owner, in ConstructData data)
            {
                Group = owner;
                Id = data.Id ?? string.Empty;
                Items = data.Items ?? Array.Empty<TItem>();
                _dict = data.Dict ?? new FakeDictionary<Id, TItem>();
            }
        ...
        public sealed class TableGroup<TItem> : TableGroup, IReadOnlyDictionary<string, Table<TItem>> where TItem : CfgTableItemBase
        {
            ...
            public struct ConstructData
            {
                ...
                public IReadOnlyDictionary<string, Table<TItem>>? TableInstDict;
            }

            public TableGroup(in ConstructData data)
            {
                Default = data.TableInstDict is not { Count: > 0 } ? new Table<TItem>(this) : data.TableInstDict.First().Value;
                Name = data.Name ?? string.Empty;
                Fields = data.Fields ?? Array.Empty<string>();
                _tableInstDict = data.TableInstDict;
            }

            public void SetDefault(Table<TItem>? table = null)
            {
                if (table == null)
                {
                    Default = _tableInstDict is not { Count: > 0 }
                        ? new Table<TItem>(this)
                        : _tableInstDict.First().Value;
                    return;
                }

                if (table.Group != this) return;
                if (_tableInstDict == null)
                {
                    Default = table;
                    return;
                }
                if (!_tableInstDict.TryGetValue(table.Id, out var t)) return;
                Default = t;
            }
            ...
            private readonly IReadOnlyDictionary<string, Table<TItem>>? _tableInstDict;
        }
